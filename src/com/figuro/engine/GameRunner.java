...
public class GameRunner implements IEngineHandler {
	...
	public GameRunner(GameJob job, IBuilder builder, IGameRules rules, IMessageSender message, IPersistency persistency) {		
		this.job = job;
    ...
    { 
		if(isGameResumable())
		{
			if(persistency.load())
			{
				...
				job.resumeGame(game, players, boardState, callback, persistency, gameType, playerTypes);
			}
		}		
		else
		{
