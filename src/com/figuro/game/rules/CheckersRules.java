...
public class CheckersRules implements IGameRules {
        ...
        for (int i = 0; i < cells1.length; ++i) {
            for (int j = 0; j < cells1[0].length; ++j) {
                ...
                if (isCellChanged(cell1, cell2) && cell1.hasUnit())
                {
                    ...
                    // else something went wrong, the unit which has been moved is not belong to the player
                }
            }
        }
    ...
    private boolean isValidMove(ICell[][] cells, Point beginPoint, Point endPoint, int player)
    {
        boolean isValidMove = false;
        ...
        if (beginCell.hasUnit() && !endCell.hasUnit())
        {
            ...
            {
                isValidMove = isValidPeasantMove(cells, beginPoint, endPoint, player);
            }
            else if (unit.getType() == UnitEnum.KING)
            {
                isValidMove = isValidKingMove(cells, beginPoint, endPoint, player);
            }
        }
    ...
    private boolean isValidPeasantMove(ICell[][] cells, Point beginPoint, Point endPoint, int player)
    {
        boolean isValidPeasantMove = false;
        
        int distance = getDistance(beginPoint, endPoint);
        if (distance == 1 && isDiagonal(beginPoint, endPoint) && isMovingAhead(player, beginPoint, endPoint))
        {
          isValidPeasantMove = true;
        }
        else if (distance == 2 && isDiagonal(beginPoint, endPoint) && isMovingAhead(player, beginPoint, endPoint) && isEnemyInBetween(cells, player, beginPoint, endPoint))
        {
          isValidPeasantMove = true;
        }
    ...
    private boolean isValidKingMove(ICell[][] cells, Point beginPoint, Point endPoint, int player)
    {
        ...
        else if (distance == 2 && isDiagonal(beginPoint, endPoint) && isEnemyInBetween(cells, player, beginPoint, endPoint))
        {
          isValidKingMove = true;
        }
    ...
    private boolean isMovingAhead(int player, Point beginPoint, Point endPoint)
    {
        ...
        if (abs(beginPoint.x - rightClosestCellPoint.x) < abs(endPoint.x - rightClosestCellPoint.x))
        {
            isMovingAhead = true;
        }
    ...
    private boolean isEnemyInBetween(ICell[][] cells, int player, Point beginPoint, Point endPoint)
    {
