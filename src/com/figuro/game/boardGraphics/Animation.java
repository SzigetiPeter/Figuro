...
public class Animation implements IAnimation {
		...
		public void handle(MouseEvent event) {
			...
			if (!selected.getId().isEmpty()) {
				...
				if ((newXPosition >= 0)
						...
						&& (newXPosition <= 1000 - ((Circle) selected
								.getChildren().get(0)).getRadius() * 2)) {
					selected.setTranslateX(newXPosition);
				}

				if ((newYPosition >= 0)
						&& (newYPosition <= 1000 - ((Circle) selected
								.getChildren().get(0)).getRadius() * 2)) {
	...
	class FinishAnimation implements EventHandler<MouseEvent> {
		...
		public void handle(MouseEvent event) {
			...
			Point finalPoint = getCheckerCoordinate(board, event.getSceneX(),
					event.getSceneY());
			NumberBinding rectsAreaSize = Bindings.min(board.getBoardPane()
					.heightProperty(), board.getBoardPane().widthProperty());
			selected.setTranslateX(finalPoint.x
					* rectsAreaSize.divide(board.getBoardSizeX()).floatValue());
	...
	private Point getCheckerCoordinate(IBoard board, double x, double y) {
		...
		for (int i = 1; i <= board.getBoardSizeX(); i++) {
			if (rectsAreaSize.divide(board.getBoardSizeX()).multiply(i)
					.floatValue() < x) {
				boardPoint.x = i;
			}
			if (rectsAreaSize.divide(board.getBoardSizeY()).multiply(i)
					.floatValue() < y) {
				boardPoint.y = i;
			}
		}
