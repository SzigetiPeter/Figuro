...
public class Board implements IBoard {
    ...
    private Group createBoard() {
        ...
        NumberBinding rectsAreaSize = Bindings.min(boarderPane.heightProperty(), boarderPane.widthProperty());

        int m = 0;
        for (int i = 0; i < this.boardSizeX; i++) {
            for (int j = 0; j < this.boardSizeY; j++) {
                Rectangle rectangle = new Rectangle(rectsAreaSize.doubleValue() / 10, rectsAreaSize.doubleValue() / 10);
                rectangle.xProperty().bind(rectsAreaSize.multiply(i).divide(this.boardSizeX));
                rectangle.yProperty().bind(rectsAreaSize.multiply(j).divide(this.boardSizeY));

                rectangle.heightProperty().bind(rectsAreaSize.divide(this.boardSizeX));
