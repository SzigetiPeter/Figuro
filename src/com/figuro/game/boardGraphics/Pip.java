...
public class Pip implements IPip {
	...
	private Circle createBoundCircle(IBoard board, int size, double center,
		...
		circle.centerXProperty().bind(
				rectsAreaSize.divide(board.getBoardSizeX() * center));
		circle.centerYProperty().bind(
				rectsAreaSize.divide(board.getBoardSizeY() * center));
		circle.radiusProperty().bind(
				rectsAreaSize.divide(board.getBoardSizeX() * radius));
	...
	private Group createPip(IBoard board, IUnit unit) {
		...
		if (unit.getType() == UnitEnum.PEASANT
				&& unit.getOwnerId() == PlayerConverter
