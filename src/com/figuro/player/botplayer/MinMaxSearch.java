...
public class MinMaxSearch implements IStepSearch {
	...
	public MinMaxSearch (IStepEvaluator stepEvaluator, IGameRules gameRules, int maxLevel) throws Exception {
		if (maxLevel < 1)
	...
	public BoardState search (BoardState current, int currentPlayerId, int opponentPlayerId) throws GameOverException {	
		...
		EvaluatedBoardState evaluatedState = deepSearch (current, _currentPlayerId, _maxLevel);
	...
	public EvaluatedBoardState deepSearch (BoardState current, int playerId, int level) {
		...
		for (BoardState possibleState : possibleStates) 
			...
			{
				EvaluatedBoardState evaluatedState = deepSearch (possibleState, otherPlayerId, level - 1);
				Evaluation evaluation = evaluatedState.getEvaluation();
				
				if (bestEvaluation == null || 
					// if it's MIN's turn then check if the current state's evaluation is less than the actual best one (evaluation < bestEvaluation)
					isCurrentPlayerId(playerId) && _stepEvaluator.min(evaluation, bestEvaluation, playerId) == evaluation ||
			   		// if it's MAX's turn then check if the current state's evaluation is larger than the actual best one (evaluation > bestEvaluation)
					!isCurrentPlayerId(playerId) && _stepEvaluator.min(bestEvaluation, evaluation, playerId) == bestEvaluation) {
